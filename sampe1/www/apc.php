...
switch($s){
	...
	case 'mod': $sort='modified_time'; $reverse=true; break;
...
$apc=new APCIterator('/'.$search.'/',APC_ITER_KEY|APC_ITER_MEM_SIZE|APC_ITER_MTIME|APC_ITER_ATIME|APC_ITER_NUM_HITS, null);

$c=0;
$now=time();
foreach($apc as $var){
    //var_export($var);
	if($var[ATIME]==$var['modified_time']) $var[ATIME]=0;
	$vars[]=$var;
	$c++;
...
foreach($vars as $var){
	...
	echo '<tr>';
		...
		echo '<td align="right">'.val(duration(time()-$var['modified_time'])).'</td>';
	echo '</tr>';
