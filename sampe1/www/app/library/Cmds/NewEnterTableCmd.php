...
class NewEnterTableCmd extends Cmd
{

    const CMD_NAME = 'NewEnterTable';
    ...
    {
        return ListTablesCmd::getTableList();
    }
    ...
    public static function genTableData()
    {
        ...
        foreach ($data_org as $b => $i) {
            $data['k'][$b] = $data['i'][$index] = array(
                ...
                'min' => $i[0],
                'max' => $i[1],
                'level' => ($index > 4 ? TABLE_LIMITATION_EXPERT : ($index > 2 ? TABLE_LIMITATION_ADVANCED : 0))
            );
            ++ $index;
        }
        return $data;
